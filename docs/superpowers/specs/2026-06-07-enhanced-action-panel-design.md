...
v4 定稿卡片结构（6 个 Widget + 卡片头部，严格按此顺序）：

**渲染规则：**
- 卡片按趋势强度评分（0-100）降序排列，高分在上
- 评分体系：趋势阶段（早期>成熟期）+ 20日动量 + 均线质量 + 量能确认 - 回调惩罚
- Widget 0/1 始终展开（最核心的趋势判断和概率推演）
- Widget 2-5 默认折叠（用 `<details>` 标签），点击标题展开
- Widget 2（买卖区间）和 Widget 4（关键价位）在展开时左右并排（grid 2列）

**标的来源与筛选：**
- 优先使用 pipeline 实时评分的 `actions_{date}.json` 中的 `etf_top5`/`stock_top5`
- 若为空，从全量池动态扫描：ETF 132只、个股200只
- 所有标的必须通过质量筛选：state≥3（偏强震荡及以上），state=3 需 pct_20d≥-3%
- state=1（下跌趋势）和 state=2（弱反弹）不进入推荐面板

**卡片头部结构：**
- 第一行：标的名 + 代码（左）| 📈 趋势评分（右）
- 第二行：💡 大白话操作标签（结合趋势阶段+动量+价格位置，零基础股民能看懂）

**变更规则：**
- 想**关闭**某个 Widget → 在配置中将 `enabled` 设为 `false`
- 想**新增** Widget → 配置加一行 + 写渲染函数
- 想**修改评分公式** → 改 `_calc_trend_score`，同步更新本文档
- 想**修改筛选门槛** → 改 `_build_card` 过滤逻辑，同步更新本文档
- **不允许**随意改变 Widget 渲染顺序或内部 HTML 结构
